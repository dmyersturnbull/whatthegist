...
"""
    ...
    def main(cls, args: Sequence[str]) -> None:
        ...
        parser.add_argument(
            ...
            "--fixes",
            action="store_true",
            help="Apply common LaTeX fixes, such as escaping reserved characters",
        )
        parser.add_argument(
            "-o",
            "--overwrite",
    ...
    def _build_bib(self) -> Mapping[str, str]:
        ...
        for doi, tag in self._iterate_bib():
            if doi in dictionary and dictionary[doi] != tag:
                logging.error(
                    "Term {} maps to both {} and {}".format(doi, dictionary[doi], tag)
                )
            dictionary[doi] = tag
        logging.info(
            "Found {} terms for {} papers".format(
        ...
        with self.bib_path.open() as f:
            for i, line in enumerate(f.readlines()):
                ...
                if line.startswith("@"):
                    if not hit_one:
                        logging.warning(
                            "{} has no global term/ID (one of {})".format(
                                tag, ",".join(Paperpiler.TERMS)
                            )
                        )
                    tag = line.split("{")[1][:-1]
                    hit_one = False
    ...
    def build_replacements(self, text: str) -> Mapping[str, str]:
        ...
        logger.info(
            "Fixed {}/{} citations for {} papers".format(
                n_found - len(cites_failed), n_found, len(refs_used)
            )
        )
        self._warn_list(refs_missed, "papers not cited")
        self._warn_list(
            cites_failed, "citations malformatted or not in the bibliography"
        )
        return fixer
